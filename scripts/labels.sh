...
declare -A LABEL_DESCRIPTIONS=(
  # From labeler.yml
  ["dependencies"]="Dependency updates (mise.toml, renovate.json5)"
  ["documentation"]="Documentation changes (README, TROUBLESHOOTING, examples)"
  ["ci"]="CI/CD pipeline changes (.github/, justfile)"
  ["ai"]="AI-related changes (CLAUDE.md, .claude/)"
  ["repo"]="Repository configuration (linters, cliff.toml, scripts/)"
  ["docker"]="Docker-related changes (Dockerfile, docker-bake.hcl)"
  # used in git-workflow claude skill
...
main() {
  ...
  # Combine and deduplicate (labeler.yml + pr-labels.yml)
  local all_labels
