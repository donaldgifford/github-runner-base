...
- Renovate (`renovate.json5`) extends shared presets from
  ...
  SHA on its next run.
- The git tag and the image tag are **not** the same string.
  `docker/metadata-action`'s `{{version}}` pattern strips the leading `v`, so
  git `v0.1.0` publishes as image `0.1.0`. Anything taking a version from a user
  must normalise it — `just verify` does, via `trim_start_match`.
- `gh attestation verify` only checks SLSA provenance unless given
  `--predicate-type`. Verifying the SBOM attestation needs a second pass, or it
  silently goes unchecked and still exits 0.
